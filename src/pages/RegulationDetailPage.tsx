...
const AddCourseModal: React.FC<{
  ...
  onClose: () => void;
  // --- CHANGE: Add existingCourse for editing ---
  existingCourse?: Course | null;
}> = ({ departmentId, regulationId, semesterNumber, onClose, existingCourse }) => {
  // --- CHANGE: Use updateCourseInRegulation for editing ---
  const { addCourseToRegulation, updateCourseInRegulation } = useData();
  const [formData, setFormData] = useState({
    name: existingCourse?.name || '',
    code: existingCourse?.code || '',
    credits: existingCourse?.credits || 3,
    weeklyHours: existingCourse?.weeklyHours || 3,
    type: existingCourse?.type || ('Theory' as 'Theory' | 'Lab' | 'Practical')
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.name && formData.code) {
      if (existingCourse) {
        ...
        updateCourseInRegulation(departmentId, regulationId, existingCourse.id, formData);
      } else {
        // Add new course
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl max-w-lg w-full mx-4">
        <div className="p-6 border-b"><h3 className="text-lg font-semibold">{existingCourse ? 'Edit Course' : `Add New Course to Semester ${semesterNumber}`}</h3></div>
        <form onSubmit={handleSubmit} className="p-6 grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 flex justify-end space-x-2 pt-4"><button type="button" onClick={onClose} className="px-4 py-2 text-sm bg-gray-100 rounded-lg">Cancel</button><button type="submit" className="px-4 py-2 text-sm text-white bg-blue-600 rounded-lg">{existingCourse ? 'Save Changes' : 'Add Course'}</button></div>
        </form>
      </div>
    </div>
  );
...
const RegulationDetailPage: React.FC = () => {
  ...
  const [modalState, setModalState] = useState<{ mode: 'add' | 'edit'; semester?: number; course?: Course } | null>(null);
  ...
  return (
     <div className="space-y-6">
              ...
              {semester.courses.length > 0 ? semester.courses.map(course => (
                 <div key={course.id} className="flex items-center justify-between p-2 rounded-lg group hover:bg-gray-50">
                   ...
                   <div className="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => setModalState({ mode: 'edit', course: course })} className="p-1 text-gray-400 hover:text-blue-600"><Edit2 className="h-4 w-4" /></button>
      ...
      {modalState?.mode === 'add' && (<AddCourseModal departmentId={deptId!} regulationId={regId!} semesterNumber={modalState.semester!} onClose={handleModalClose}/>)}
      {modalState?.mode === 'edit' && (<AddCourseModal departmentId={deptId!} regulationId={regId!} semesterNumber={0} existingCourse={modalState.course} onClose={handleModalClose}/>)}
